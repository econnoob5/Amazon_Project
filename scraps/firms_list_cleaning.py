...
DESCRIPTION: the following script takes in as input the excel files with public company names (downloaded from NASDAQ's 
             ...
             applied, as in this case, to deal with imperfect matching_firms_and_brands between the strings (company names).
...
# extract directory of "from_nasdaq_to_clean" folder
directory_to_clean = os.path.join(project_dir, "Amazon Project - Data/Firms lists from NASDAQ/from_nasdaq_to_clean")

# extract directory of "Amazon Project - Data" folder
dir_cleaned = os.path.dirname(os.path.dirname(directory_to_clean))
# extract directory of "from_nasdaq_cleaned" folder
directory_cleaned = os.path.join(dir_cleaned, "Firms lists from NASDAQ/from_nasdaq_cleaned")
...
for filename in os.listdir(directory_to_clean):
    ...
    #     r"Firms lists from NASDAQ/from_nasdaq_cleaned/"
    #     r"Companies dropped/" + "Names_dropped_" + name_dataset[15:] + ".csv")
    # dataset.to_csv(
    #     r"/Users/mattia/OneDrive - IESE Business School/Documentos/Amazon Project/Amazon Project - Data/"
    #     r"Firms lists from NASDAQ/from_nasdaq_cleaned/" + name_dataset + "_" + "nodup" + ".csv",
    #     columns=['Symbol', 'Name', 'Market Cap', 'Country', 'Sector', 'Industry'], index=False)
