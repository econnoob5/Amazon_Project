    ...
    for row in df.itertuples():
        ...
        for line in df_csv_file.itertuples():
            ...
            for item in brand_list:
                for item2 in product_name_list:
                    fuzz_ratio = fuzz.ratio(item.lower(), item2.lower())
                    ratio_list.append(fuzz_ratio)
                    if fuzz_ratio > 60:
                        tuple_ratios = (str(row[0]), file[:-5], str(brand), str(product_name), str(fuzz_ratio))
