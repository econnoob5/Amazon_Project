...
for file in os.listdir(dir_load):
    ...
    def matching(brand_item, amazon_brands):
        ...
        if brand_item != None:
            ...
            ratio = np.array(list(map(lambda x: fuzz.ratio(x, brand_item), list(factset_products_list))))
            # The output will be a list of ratio, one for each element in factset_products_list

            if np.max(ratio, initial=1) > 80:  # We can play with this number
                ...
                # Given that 'factset_products_list' and 'ratio' have the same length (as the second is built from
                # the first) we join them on a single dataframe called 'a'
                a = pd.DataFrame({"factset_product": factset_products_list, "ratio": ratio},
                ...
                match_score = list(matches['ratio'])
    ...
    output_df_json['ratio'] = flattened_match_score
    output_df_json['category'] = file[5:-5]
    # append to final df
    output_df_final = output_df_final.append(output_df_json, ignore_index=True)

# split the dfs for saving in excel/csv, otherwise too many rows
