...
for file in os.listdir(dir_load):
    ...
    def matching(brand_item, amazon_brands):
        ...
        if brand_item != None:
            ...
            sortratios = np.array(list(map(lambda x: fuzz.token_sort_ratio(x, brand_item), list(factset_products_list))))
            # The output will be a list of sortratios, one for each element in factset_products_list

            if np.max(sortratios, initial=1) > 80:  # We can play with this number
                ...
                # Given that 'factset_products_list' and 'sortratios' have the same length (as the second is built from
                # the first) we join them on a single dataframe called 'a'
                a = pd.DataFrame({"factset_product": factset_products_list, "sort_ratio": sortratios},
                                 columns=["factset_product", "sort_ratio"])

                # We get the factset_product with the max ratio, it will be our match
                match = a.loc[a.sort_ratio == a.sort_ratio.max(), "factset_product"].values[0]

                match_score = a.loc[a.sort_ratio == a.sort_ratio.max(), "sort_ratio"].values[0]
                ...
                output_df_json.append({"amazon": brand_item, "factset": match, "sort_ratio": match_score})
            else:
    ...
    output_df_json = pd.DataFrame(output_df_json, columns=["amazon", "factset", "sort_ratio"])
...
output_df_final.to_csv(r"/data/users/mlaudi/Amazon/Amazon Project - Data/Results/matching_output_fSortratio.csv", index=False)
output_df_final.to_excel(r"/data/users/mlaudi/Amazon/Amazon Project - Data/Results/matching_output_fSortratio.xlsx", index=False)
...
for threshold in range(80, 101):
    thr = threshold
    filter1 = output_df_final.loc[output_df_final['sort_ratio'] >= threshold]
...
plt.savefig(dir_save + '/matches_sort_ratio.pdf', bbox_inches='tight')
