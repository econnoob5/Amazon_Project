"""
Code PURPOSE: replicating tables 1-3 in the Customer Knows Best paper
...
dir_load = os.path.join(PROJECT_ROOT_DIR, TARGET_FILE_ROOT_DIR)
...
# average_monthly_rating = pd.read_excel(r"../Amazon Project - Data/support_data/Miscellaneous/average_overall_wratio.xlsx")
# number_reviews = pd.read_excel(r"../Amazon Project - Data/support_data/Miscellaneous/number_reviews_wratio.xlsx")
...
for row3 in average_overall_merged_10.itertuples():
    ...
    for i in range(len(row3[14:])):
        ...
        for x in range(1, 13):  # .columns does not count the index column.
            ...
            if row3[14 + i - x] != 'nan':
                grand_total += number_reviews_merged_10[average_overall_merged_10.columns[13 + i - x]][
                                   list(number_reviews_merged_10['LPERMNO']).index(row3[1])] * row3[14 + i - x]
                number_rate += number_reviews_merged_10[average_overall_merged_10.columns[13 + i - x]][
                    list(number_reviews_merged_10['LPERMNO']).index(row3[1])]
        if number_rate != 0:
...
# 'final_portfolio10', keeps track, each month, of the permnos that are in each portfolio
# 'final_portfolio10_c', keeps track of the count of the reviews each permno of each tercile has received in a given month

final_portfolio10 = pd.DataFrame(columns=('Month', 'T1', 'T2', 'T3'))
final_portfolio10_c = pd.DataFrame(columns=('Month', 'T1', 'T2', 'T3'))
...
for month in islice(abnormal_matrix.transpose().itertuples(), 1, None):
            ...
            try:
                ...
                counts_ratings.append(
                    number_reviews[str(month[MONTHYEAR_ABNORMAL_MATRIX])][
                        list(number_reviews['LPERMNO']).index(all_permnos[i])])
            except:
    ...
    if len(abnormal) != 0:
        ...
        prov = pd.Series(portfolios, index=final_portfolio10.columns)
        final_portfolio10 = final_portfolio10.append(prov, ignore_index=True)
        ...
        prov_c = pd.Series(portfolios_c, index=final_portfolio10_c.columns)
        final_portfolio10_c = final_portfolio10_c.append(prov_c, ignore_index=True)

# save final_portfolio10 and final_portfolio10_c for debugging
final_portfolio10.to_csv(
    r"../Amazon Project - Data\support_data\CKB replication/final_portfolio_wratio_10votes.csv", index=False)
print("final_portfolio10 done")

final_portfolio10_c.to_csv(
    r"../Amazon Project - Data\support_data\CKB replication/final_portfolio_c_wratio_10votes.csv", index=False)
print("final_portfolio10_c done")
...
# load 'number_reviews_merged_10'
number_reviews_merged_10 = pd.read_excel(r"..\Amazon Project - Data\support_data\Miscellaneous\permnos_wratio_10votes.xlsx")
mask10 = permnos_returns.PERMNO.isin(number_reviews_merged_10.LPERMNO)
permnos_returns10 = permnos_returns[mask10]
...
final_portfolio10 = pd.read_csv(
    r"../../Amazon Project - Data/support_data/CKB replication/final_portfolio_wratio_10votes.csv")
final_portfolio10_c = pd.read_csv(
    r"../../Amazon Project - Data/support_data/CKB replication/final_portfolio_c_wratio_10votes.csv")


# the idea now could be to go in 'final_portfolio10', extract each month the permnos that are in each portfolio and go
# into 'permnos_returns10' and join if yyyymm and permnos are the same in both. We could have for output a df where:
...
date_T1_permnos['Month'] = final_portfolio10['Month']
date_T1_permnos['T1'] = final_portfolio10['T1'].str[1:-1]

date_T2_permnos['Month'] = final_portfolio10['Month']
date_T2_permnos['T2'] = final_portfolio10['T2'].str[1:-1]

date_T3_permnos['Month'] = final_portfolio10['Month']
date_T3_permnos['T3'] = final_portfolio10['T3'].str[1:-1]
...
date_T1_count['Month'] = final_portfolio10_c['Month']
date_T1_count['T1'] = final_portfolio10_c['T1'].str[1:-1]

date_T2_count['Month'] = final_portfolio10_c['Month']
date_T2_count['T2'] = final_portfolio10_c['T2'].str[1:-1]

date_T3_count['Month'] = final_portfolio10_c['Month']
date_T3_count['T3'] = final_portfolio10_c['T3'].str[1:-1]
...
final_T1 = df_T1.merge(permnos_returns10, how='inner', left_on=['Month', 'T1'], right_on=['date', 'PERMNO']).merge(
...
final_T2 = df_T2.merge(permnos_returns10, how='inner', left_on=['Month', 'T2'], right_on=['date', 'PERMNO']).merge(
    factors_4CFF, how='inner', left_on='Month', right_on='date')
...
final_T3 = df_T3.merge(permnos_returns10, how='inner', left_on=['Month', 'T3'], right_on=['date', 'PERMNO']).merge(
    factors_4CFF, how='inner', left_on='Month', right_on='date')
...
T1_portfolio.to_stata(r'../Amazon Project - Data/Results/T1_portfolio_10rev.dta', write_index=False)
T2_portfolio.to_stata(r'../Amazon Project - Data/Results/T2_portfolio_10rev.dta', write_index=False)
T3_portfolio.to_stata(r'../Amazon Project - Data/Results/T3_portfolio_10rev.dta', write_index=False)
...
final_T1_weight = df_T1_sum_reviews.merge(permnos_returns10, how='inner', left_on=['Month', 'T1'], right_on=['date', 'PERMNO']).merge(
    factors_4CFF, how='inner', left_on='Month', right_on='date')
...
final_T2_weight = df_T2_sum_reviews.merge(permnos_returns10, how='inner', left_on=['Month', 'T2'], right_on=['date', 'PERMNO']).merge(
    factors_4CFF, how='inner', left_on='Month', right_on='date')
...
final_T3_weight = df_T3_sum_reviews.merge(permnos_returns10, how='inner', left_on=['Month', 'T3'], right_on=['date', 'PERMNO']).merge(
    factors_4CFF, how='inner', left_on='Month', right_on='date')
...
T1_portfolio_w.to_stata(r'../Amazon Project - Data/Results/T1_portfolio_weighted_10rev.dta', write_index=False)
T2_portfolio_w.to_stata(r'../Amazon Project - Data/Results/T2_portfolio_weighted_10rev.dta', write_index=False)
T3_portfolio_w.to_stata(r'../Amazon Project - Data/Results/T3_portfolio_weighted_10rev.dta', write_index=False)
...
long_short_portfolio.to_stata(r'../Amazon Project - Data/Results/long_short_portfolio_10rev.dta', write_index=False)
...
long_short_portfolio_w.to_stata(r'../Amazon Project - Data/Results/long_short_portfolio_weighted_10rev.dta', write_index=False)
